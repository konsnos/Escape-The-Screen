...
namespace EscapeTheScreen
{
    public class Main : MonoBehaviour
    {
        ...
        {
            var keyUpReturn = Input.GetKeyUp(KeyCode.Return);
            var keyUpKeypadEnter = Input.GetKeyUp(KeyCode.KeypadEnter);
            var keyUpSpace = Input.GetKeyUp(KeyCode.Space);
            if (!keyUpReturn && !keyUpKeypadEnter && !keyUpSpace) return;
            
            switch (activeScreen)
            {
                case SCREEN_STATES.DESKTOP:
                    CheckIconButtonClickDesktop();
                    break;
                ...
                case SCREEN_STATES.UNSORTED:
                    CheckIconButtonClickUnsorted();
                    break;
                case SCREEN_STATES.DRIVERS:
                    CheckIconButtonClickDrivers();
                    break;
        ...
        private void CheckIconButtonClickDrivers()
        {
            switch (IconHandler.selectedBtn)
            {
        ...
        private void CheckIconButtonClickUnsorted()
        {
            switch (IconHandler.selectedBtn)
            {
                ...
                case BUTTONS.PACMAN_DOC:
                    activeScreen = SCREEN_STATES.PACMAN_DOC;
        ...
        private void CheckIconButtonClickDesktop()
        {
            switch (IconHandler.selectedBtn)
            {
                case BUTTONS.RECYCLE_BIN:
                ...
                case BUTTONS.USER:
                    ...
                    else
                    {
                        ...
                        OpenSmthSnd.Play();
                        UserPassInput.text = string.Empty;
                    }
        ...
        private void checkUserPassInput(bool check)
        {
            ...
            var keyUpReturn = Input.GetKeyUp(KeyCode.Return);
            var keyUpKeypadReturn = Input.GetKeyUp(KeyCode.KeypadEnter);
            if (!keyUpReturn && !keyUpKeypadReturn && !check) return;
